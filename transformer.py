...
class EncoderDecoder(nn.Module):
    def __init__(self,encoder,decoder,src_embed,tgt_embed,generator):
        ...
        self.encoder=encoder  #是Encoder的实例
...
#训练记账本
class TrainState:
    step: int = 0  #当前轮次的更新步数
    accum_step:int=0 #梯度累计步数
    samples:int=0
    tokens:int=0
...
def run_epoch(
    ...
    for i,batch in enumerate(data_iter):
        ...
        if i%40==1 and (mode=="train" or mode=="train+log"):
            ...
            print(
                ("Epoch Step:%6d | Accumulation Step:%3d | Loss:%6.2f"+" | Tokens /Sec:%7.1f | Learning Rate:%6.6f")
...
def greedy_decode(model,src,src_mask,max_len,start_symbol):
    ...
    ys=torch.zeros(1,1).fill_(start_symbol).type_as(src.data)  #初始化输出并用开始符填充
...
if __name__ =="__main__":
    example_simple_model()   #导入时不调用训练函数